          ...
          {messages.map((message, index) => (
            <Box key={index} alignSelf={message.role === "user" ? "flex-start" : "flex-end"}>
              ...
              {message.text && message.text.includes("```") ? (
                <Box p={2} bg={message.role === "user" ? "blue.100" : "green.100"} borderRadius="md">
                  {message.text.split("\n").map((line, idx) =>
                    line.startsWith("```") ? (
                      <Code key={idx} p={2} bg={message.role === "user" ? "blue.50" : "green.50"} borderRadius="md" children={line.replace(/```/g, "")} />
                    ) : (
